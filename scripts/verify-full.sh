...
echo "==> [3/5] smoke:roles"
BASE_URL="http://127.0.0.1:5173" node scripts/role-nav-smoke.mjs || \
  BASE_URL="http://127.0.0.1:4173" node scripts/role-nav-smoke.mjs || true

echo "==> [4/5] build"
npm run build

echo "==> [5/5] smoke:prod (geçici :4178)"
...
echo "==> [bonus] e2e:campus (kendi :4177 sunucusu)"
