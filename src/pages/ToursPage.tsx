...
export default function ToursPage() {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Tur Masası</h1>
        <p className="mt-1 text-sm text-slate-400">Tur / gezi rezervasyonu.</p>
      </header>
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchTours(r.id,{status:'booked'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>booked</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchTours(r.id,{status:'departed'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>departed</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchTours(r.id,{status:'cancelled'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>cancelled</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
