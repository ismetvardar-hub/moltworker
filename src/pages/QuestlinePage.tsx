...
import { createQuestline, fetchQuestline, patchQuestline } from '../services/questline'
export default function QuestlinePage() {
  ...
  async function refresh() {
    try { const data = await fetchQuestline(); setRows(data.questline || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createQuestline(payload); await refresh()
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <CrudOpsBar domain="questline" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchQuestline(r.id,{status:'ok'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>ok</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchQuestline(r.id,{status:'warn'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>warn</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchQuestline(r.id,{status:'alarm'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>alarm</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
