...
import { createJetski, fetchJetski, patchJetski } from '../services/jetski'
export default function JetskiPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"unit":"JS-01","guestName":"Misafir"})
  async function refresh() {
    try { const data = await fetchJetski(); setRows(data.jetski || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createJetski(payload); await refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Kayıt başarısız') }
  }
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Jet Ski</h1>
        <p className="mt-1 text-sm text-slate-400">Jet ski kiralama.</p>
      </header>
      <CrudOpsBar domain="jetski" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchJetski(r.id,{status:'available'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>available</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchJetski(r.id,{status:'rented'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>rented</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchJetski(r.id,{status:'service'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>service</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
