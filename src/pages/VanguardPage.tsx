...
import * as api from '../services/vanguard'
...
export default function VanguardPage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchVanguard())
      setError(null)
    } catch (e) {
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runVanguardSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.forceVanguardPosOnline({}).then((r: any) => {
                    ping(`POS online ${r.updated?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.clearVanguardInvDrift({ force: true }).then((r: any) => {
                    ping(`Inv clear ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.flushVanguardMintQueue({}).then((r: any) => {
                    ping(`Mint flush ${r.flushed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackVanguardFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · POS offline {data.summary?.pos_offline ?? 0} · mint{' '}
              {data.summary?.mint_queued ?? 0} · drift {data.summary?.inv_drift ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackVanguardFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
