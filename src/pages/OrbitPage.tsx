...
import * as api from '../services/orbit'
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Orbit</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 135 — oda · kart · misafir yörüngesi.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runOrbitSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.cleanOrbitRooms({}).then((r: any) => {
                    ping(`Room ${r.cleaned?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.encodeOrbitKeys({}).then((r: any) => {
                    ping(`Key ${r.encoded?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.retryOrbitGuestapp({}).then((r: any) => {
                    ping(`App ${r.sent?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackOrbitFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">Flag {data.summary?.flags_open ?? 0} · dirty {data.summary?.dirty_rooms ?? 0} · key {data.summary?.key_queued ?? 0} · QR {data.summary?.qr_pending ?? 0}</p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackOrbitFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
