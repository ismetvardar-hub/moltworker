...
import * as api from '../services/apex'
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Apex</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 150 — finans · uyum · operasyon zirvesi.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runApexSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.clearApexInvoices({}).then((r: any) => {
                    ping(`Invoice ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.renewApexLicenses({}).then((r: any) => {
                    ping(`License ${r.renewed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.approveApexOvertime({}).then((r: any) => {
                    ping(`Overtime ${r.approved?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackApexFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · overdue {data.summary?.invoices_overdue ?? 0} · lic {data.summary?.licenses_expiring ?? 0} · sla {data.summary?.sla_open ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackApexFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
