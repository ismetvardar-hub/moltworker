...
import * as api from '../services/ledger'
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-400">AŞAMA 255 — AR/AP · iade · kanal finansı.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runLedgerSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.collectLedgerAr({}).then((r: any) => {
                    ping(`AR ${r.collected?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.resolveLedgerChargeback({}).then((r: any) => {
                    ping(`CB ${r.resolved?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.clearLedgerChannel({}).then((r: any) => {
                    ping(`Channel ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackLedgerFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">Flag {data.summary?.flags_open ?? 0} · AR {data.summary?.ar_overdue ?? 0} · AP {data.summary?.ap_held ?? 0} · CB {data.summary?.chargeback_open ?? 0}</p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackLedgerFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
