...
import * as api from '../services/bastion'
...
export default function BastionPage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchBastion())
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Bastion</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 705 — erişim · kimlik · ihlal kalkanı.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runBastionSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.closeBastionAccess({}).then((r: any) => {
                    ping(`Access ${r.closed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.approveBastionRole({}).then((r: any) => {
                    ping(`Role ${r.approved?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.archiveBastionBreach({}).then((r: any) => {
                    ping(`Breach ${r.archived?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackBastionFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · access {data.summary?.access_open ?? 0} · role {data.summary?.role_draft ?? 0} · breach {data.summary?.breach_draft ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackBastionFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
