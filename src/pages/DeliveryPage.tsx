          ...
          {rows.map((r) => (
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1">
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={() => void patchDelivery(r.id, { status: 'preparing' }).then(() => refresh()).catch((e) => setError(String(e.message || e)))}>preparing</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={() => void patchDelivery(r.id, { status: 'ready' }).then(() => refresh()).catch((e) => setError(String(e.message || e)))}>ready</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={() => void patchDelivery(r.id, { status: 'delivered' }).then(() => refresh()).catch((e) => setError(String(e.message || e)))}>delivered</button>
              </div>
            </li>
          ))}
        </ul>
