  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <PanelCard title={overview?.title || 'Olay ops'}>
          <ul className="list-disc space-y-1 pl-5 text-sm text-slate-300">{(overview?.summaryLines || []).map((l: string) => (<li key={l}>{l}</li>))}</ul>
          <div className="mt-3 flex flex-wrap gap-2">
            <button type="button" className="rounded-lg bg-lykia-500/90 px-3 py-2 text-sm text-obsidian-950" onClick={() => void api.runIncidentsSweep({ force: true }).then((r: any) => { ping(`Sweep +${r.created?.length ?? 0}`); return refresh() })}>Sweep</button>
            <button type="button" className="rounded-lg bg-rose-500/20 px-3 py-2 text-sm text-rose-100" onClick={() => void api.ackOpenCriticalIncidents({}).then((r: any) => { ping(`Ack crit ${r.acked?.length ?? 0}`); return refresh() })}>Ack critical</button>
            <button type="button" className="rounded-lg bg-amber-500/20 px-3 py-2 text-sm text-amber-100" onClick={() => void api.resolveOpenIncidents({}).then((r: any) => { ping(`Resolve ${r.resolved?.length ?? 0}`); return refresh() })}>Resolve open</button>
            <button type="button" className="rounded-lg bg-sky-500/20 px-3 py-2 text-sm text-sky-100" onClick={() => void api.escalateIncidentSeverity({}).then((r: any) => { ping(`Escalate ${r.escalated?.length ?? 0}`); return refresh() })}>Escalate</button>
            <button type="button" className="rounded-lg bg-obsidian-800 px-3 py-2 text-sm" onClick={() => void api.ackIncidentsFlag({}).then((r: any) => { ping(r.ok ? 'Flag ack' : r.error || 'Ack yok'); return refresh() })}>Flag ack</button>
          </div>
          <p className="mt-2 text-xs text-slate-500">Flag {overview?.summary?.flags_open ?? 0} · open {overview?.summary?.open ?? open} · critical {overview?.summary?.critical ?? critical}</p>
        </PanelCard>
          ...
          <ul className="space-y-2 text-sm">
            ...
            {(overview?.flags || []).slice(0, 10).map((f: any) => (
              <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                ...
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px]" onClick={() => void api.ackIncidentsFlag({ id: f.id }).then(() => { ping('Ack'); return refresh() })}>Ack</button>
              </li>
            ))}
          </ul>
        </PanelCard>
