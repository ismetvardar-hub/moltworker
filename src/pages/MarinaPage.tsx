...
export default function MarinaPage() {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Marina</h1>
        <p className="mt-1 text-sm text-slate-400">Tekne bağlama / slip.</p>
      </header>
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMarina(r.id,{status:'free'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>free</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMarina(r.id,{status:'occupied'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>occupied</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMarina(r.id,{status:'hold'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>hold</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
