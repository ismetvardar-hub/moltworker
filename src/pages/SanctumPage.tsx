...
import * as api from '../services/sanctum'
...
export default function SanctumPage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchSanctum())
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Sanctum</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 480 — spa · wellness · biyometrik özeti.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runSanctumSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.clearSanctumSpa({}).then((r: any) => {
                    ping(`Spa ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.clearSanctumBio({}).then((r: any) => {
                    ping(`Bio ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.completeSanctumSession({}).then((r: any) => {
                    ping(`Session ${r.completed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackSanctumFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">Flag {data.summary?.flags_open ?? 0} · spa {data.summary?.spa_in_cabin ?? 0} · thermal {data.summary?.thermal_busy ?? 0} · bio {data.summary?.bio_flagged ?? 0}</p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackSanctumFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
