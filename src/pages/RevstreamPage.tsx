...
import { createRevstream, fetchRevstream, patchRevstream } from '../services/revstream'
export default function RevstreamPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"channel":"F&B","amount":"52000"})
  async function refresh() {
    try { const data = await fetchRevstream(); setRows(data.revstream || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createRevstream(payload); await refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Kayıt başarısız') }
  }
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Gelir Akışı</h1>
        <p className="mt-1 text-sm text-slate-400">Kanal bazlı gelir akış satırları.</p>
      </header>
      <CrudOpsBar domain="revstream" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchRevstream(r.id,{status:'live'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>live</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchRevstream(r.id,{status:'locked'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>locked</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
