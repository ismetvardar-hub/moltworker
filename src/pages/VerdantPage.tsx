...
import * as api from '../services/verdant'
...
export default function VerdantPage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchVerdant())
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-400">AŞAMA 690 — karbon · su · ESG yeşili.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runVerdantSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.clearVerdantWater({}).then((r: any) => {
                    ping(`Water ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.closeVerdantEsg({}).then((r: any) => {
                    ping(`ESG ${r.closed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.fixVerdantEv({}).then((r: any) => {
                    ping(`EV ${r.fixed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackVerdantFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">Flag {data.summary?.flags_open ?? 0} · water {data.summary?.water_alarm ?? 0} · solar {data.summary?.solar_fault ?? 0} · ESG {data.summary?.esg_open ?? 0}</p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackVerdantFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
