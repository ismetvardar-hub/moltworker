...
import { createWayfind, fetchWayfind, patchWayfind } from '../services/wayfind'
export default function WayfindPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"point":"Beach Gate","label":"Sahil"})
  async function refresh() {
    try { const data = await fetchWayfind(); setRows(data.wayfind || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createWayfind(payload); await refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Kayıt başarısız') }
  }
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Wayfinding</h1>
        <p className="mt-1 text-sm text-slate-400">Yönlendirme noktaları.</p>
      </header>
      <CrudOpsBar domain="wayfind" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchWayfind(r.id,{status:'active'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>active</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchWayfind(r.id,{status:'moved'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>moved</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchWayfind(r.id,{status:'offline'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>offline</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
