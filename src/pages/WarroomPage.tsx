...
import * as api from '../services/warroom'
...
export default function WarroomPage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchWarroom())
      setError(null)
    } catch (e) {
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runWarroomSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.clearWarroomHaccp({}).then((r: any) => {
                    ping(`HACCP clear ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.clearWarroomPatrol({}).then((r: any) => {
                    ping(`Patrol clear ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.closeWarroomConcierge({}).then((r: any) => {
                    ping(`Concierge close ${r.closed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackWarroomFlag({}).then((r: any) => {
            ...
            <p className="mt-3 text-xs text-slate-500">
              Hazırlık {data.boardpack?.readiness?.overall} · {data.boardpack?.readiness?.grade} · flag{' '}
              {data.summary?.flags_open ?? 0} · HACCP {data.summary?.haccp_fail ?? 0} · patrol{' '}
              {data.summary?.patrol_alert ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackWarroomFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
