...
import * as api from '../services/aegis'
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Aegis</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 525 — güvenlik · uyumluluk · tatbikat özeti.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runAegisSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.clearAegisSafety({}).then((r: any) => {
                    ping(`Safety clear ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.closeAegisIncident({}).then((r: any) => {
                    ping(`Incident close ${r.closed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.clearAegisEvac({}).then((r: any) => {
                    ping(`Evac clear ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackAegisFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · safety {data.summary?.safety_open ?? 0} · inc{' '}
              {data.summary?.inc_investigating ?? 0} · evac {data.summary?.evac_blocked ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackAegisFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
