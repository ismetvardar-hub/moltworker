...
import { createLostchild, fetchLostchild, patchLostchild } from '../services/lostchild'
export default function LostchildPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"childName":"Ada","zone":"Kids"})
  async function refresh() {
    try { const data = await fetchLostchild(); setRows(data.lostchild || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createLostchild(payload); await refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Kayıt başarısız') }
  }
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Kayıp Çocuk</h1>
        <p className="mt-1 text-sm text-slate-400">Kayıp çocuk protokol log.</p>
      </header>
      <CrudOpsBar domain="lostchild" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchLostchild(r.id,{status:'open'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>open</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchLostchild(r.id,{status:'found'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>found</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchLostchild(r.id,{status:'closed'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>closed</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
