...
import { createGiftred, fetchGiftred, patchGiftred } from '../services/giftred'
export default function GiftredPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"code":"GIFT-10","amount":"250"})
  async function refresh() {
    try { const data = await fetchGiftred(); setRows(data.giftred || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createGiftred(payload); await refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Kayıt başarısız') }
  }
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Hediye Kullanım</h1>
        <p className="mt-1 text-sm text-slate-400">Gift card redeem log.</p>
      </header>
      <CrudOpsBar domain="giftred" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchGiftred(r.id,{status:'redeemed'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>redeemed</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchGiftred(r.id,{status:'void'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>void</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
