...
import * as api from '../services/ecosphere'
...
export default function EcospherePage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchEcosphere())
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Ecosphere</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 285 — ESG · uyum · sürdürülebilirlik özeti.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runEcosphereSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.mitigateEcosphereFinding({}).then((r: any) => {
                    ping(`Finding ${r.mitigated?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.fulfillEcosphereDataprotect({}).then((r: any) => {
                    ping(`KVKK ${r.fulfilled?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.clearEcosphereVendorHigh({}).then((r: any) => {
                    ping(`Vendor clear ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackEcosphereFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · findings {data.summary?.findings_open ?? 0} · kvkk{' '}
              {data.summary?.dataprotect_open ?? 0} · vendor {data.summary?.vendor_high ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackEcosphereFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
