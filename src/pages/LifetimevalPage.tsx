...
import { createLifetimeval, fetchLifetimeval, patchLifetimeval } from '../services/lifetimeval'
export default function LifetimevalPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"guestName":"Misafir","value":"42000"})
  async function refresh() {
    try { const data = await fetchLifetimeval(); setRows(data.lifetimeval || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createLifetimeval(payload); await refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Kayıt başarısız') }
  }
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">LTV</h1>
        <p className="mt-1 text-sm text-slate-400">Ömür boyu değer tahmini.</p>
      </header>
      <CrudOpsBar domain="lifetimeval" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchLifetimeval(r.id,{status:'estimated'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>estimated</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchLifetimeval(r.id,{status:'confirmed'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>confirmed</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
