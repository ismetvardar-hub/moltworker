...
import * as api from '../services/citadel'
...
export default function CitadelPage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchCitadel())
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Citadel</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 495 — tesis · enerji · iş emri özeti.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runCitadelSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.clearCitadelPlant({}).then((r: any) => {
                    ping(`Plant ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.clearCitadelHvac({}).then((r: any) => {
                    ping(`HVAC ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.closeCitadelWorkorder({}).then((r: any) => {
                    ping(`WO ${r.closed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackCitadelFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · plant {data.summary?.plant_critical ?? 0} · hvac {data.summary?.hvac_fault ?? 0} · wo {data.summary?.wo_open ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackCitadelFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
