...
import * as api from '../services/peoplehub'
...
export default function PeoplehubPage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchPeoplehub())
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">People Hub</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 270 — İK · izin · güvenlik kültürü.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runPeoplehubSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.approvePeoplehubLeave({}).then((r: any) => {
                    ping(`Leave ${r.approved?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.closePeoplehubNearmiss({}).then((r: any) => {
                    ping(`Near miss ${r.closed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.completePeoplehubOnboarding({}).then((r: any) => {
                    ping(`Onboarding ${r.completed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackPeoplehubFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · leave {data.summary?.leave_requested ?? 0} · near {data.summary?.near_miss ?? 0} · whistle {data.summary?.whistle_open ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackPeoplehubFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
