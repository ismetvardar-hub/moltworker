...
export default function MinibarPage() {
  ...
  async function refresh() {
    try {
      const data = await fetchMinibar()
      setRows(data.minibar || [])
      setError(null)
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMinibar(r.id,{status:'pending'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>pending</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMinibar(r.id,{status:'restocked'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>restocked</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMinibar(r.id,{status:'billed'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>billed</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
