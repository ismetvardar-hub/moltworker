...
import * as api from '../services/boardpack'
...
export default function BoardpackPage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchBoardpack())
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Board Pack</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 90 — yönetim paketi.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runBoardpackSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.snapshotBoardpack({}).then((r: any) => {
                    ping(`Snapshot ${r.snapshot?.id || 'ok'}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.renewBoardpackContract({}).then((r: any) => {
                    ping(`Contract ${r.renewed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.rebalanceBoardpackBudget({}).then((r: any) => {
                    ping(`Budget ${r.rebalanced?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackBoardpackFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · hazırlık {data.summary?.readiness ?? 0}/
              {data.summary?.grade ?? '-'} · renew {data.summary?.renewing ?? 0} · overspend{' '}
              {data.summary?.overspend ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackBoardpackFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
