...
import * as api from '../services/brandpulse'
...
export default function BrandpulsePage() {
  ...
  async function refresh() {
    try {
      setData(await api.fetchBrandpulse())
      setError(null)
    } catch (e) {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Brand Pulse</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 300 — marka · içerik · sosyal nabız.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runBrandpulseSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.triageBrandpulseInbox({}).then((r: any) => {
                    ping(`Inbox triage ${r.replied?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.approveBrandpulseUgc({}).then((r: any) => {
                    ping(`UGC approve ${r.approved?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.actionBrandpulseGuard({}).then((r: any) => {
                    ping(`Guard action ${r.actioned?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackBrandpulseFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · inbox {data.summary?.inbox_new ?? 0} · ugc{' '}
              {data.summary?.ugc_queued ?? 0} · watch {data.summary?.brand_watching ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackBrandpulseFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
