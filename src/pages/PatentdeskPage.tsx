...
import { createPatentdesk, fetchPatentdesk, patchPatentdesk } from '../services/patentdesk'
export default function PatentdeskPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"title":"Alpha","status":"Beta"})
  async function refresh() {
    try { const data = await fetchPatentdesk(); setRows(data.patentdesk || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createPatentdesk(payload); await refresh()
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <CrudOpsBar domain="patentdesk" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchPatentdesk(r.id,{status:'queued'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>queued</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchPatentdesk(r.id,{status:'running'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>running</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchPatentdesk(r.id,{status:'done'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>done</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
