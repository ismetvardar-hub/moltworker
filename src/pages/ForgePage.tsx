...
import * as api from '../services/forge'
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Forge</h1>
        <p className="mt-1 text-sm text-slate-400">AŞAMA 510 — yetenek · vardiya · kadro özeti.</p>
      </header>
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runForgeSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.advanceForgeTalent({}).then((r: any) => {
                    ping(`Talent ${r.advanced?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.renewForgeCert({}).then((r: any) => {
                    ping(`Cert renew ${r.renewed?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.approveForgeShift({}).then((r: any) => {
                    ping(`Shift approve ${r.approved?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackForgeFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · offer {data.summary?.talent_offer ?? 0} · cert{' '}
              {data.summary?.cert_expiring ?? 0} · swap {data.summary?.swap_requested ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackForgeFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
