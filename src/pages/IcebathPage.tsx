...
import { createIcebath, fetchIcebath, patchIcebath } from '../services/icebath'
export default function IcebathPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"slot":"09:00","guestName":"Misafir"})
  async function refresh() {
    try { const data = await fetchIcebath(); setRows(data.icebath || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createIcebath(payload); await refresh()
    } catch (err) { setError(err instanceof Error ? err.message : 'Kayıt başarısız') }
  }
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Buz Banyo</h1>
        <p className="mt-1 text-sm text-slate-400">Ice bath rezervasyon.</p>
      </header>
      <CrudOpsBar domain="icebath" onDone={() => void refresh()} />
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchIcebath(r.id,{status:'booked'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>booked</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchIcebath(r.id,{status:'done'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>done</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchIcebath(r.id,{status:'no_show'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>no_show</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
