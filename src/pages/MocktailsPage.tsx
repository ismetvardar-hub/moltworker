...
export default function MocktailsPage() {
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-bold tracking-tight text-lykia-200">Mocktail Bar</h1>
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMocktails(r.id,{status:'ordered'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>ordered</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMocktails(r.id,{status:'served'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>served</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchMocktails(r.id,{status:'void'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>void</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
