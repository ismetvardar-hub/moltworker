...
} from '../services/otareviews'

export default function OtareviewsPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [flash, setFlash] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"channel":"Booking","score":"9"})
  async function refresh() {
    try {
      const data = await fetchOtareviews()
      setRows(data.otareviews || [])
      setError(null)
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createOtareviews(payload)
      await refresh()
    ...
    <div className="space-y-6 p-6">
      ...
      <PanelCard title="Wave 175 ops">
        <div className="flex flex-wrap gap-2">
          <button type="button" className="rounded-lg bg-lykia-500/90 px-3 py-2 text-sm text-obsidian-950" onClick={()=>void runOp('Sweep', () => runOtareviewsSweep({ force: true }))}>Sweep</button>
          <button type="button" className="rounded-lg bg-amber-500/20 px-3 py-2 text-sm text-amber-100" onClick={()=>void runOp('Low score', () => markOtareviewsLowScoreSpike({ score: 4 }))}>Low score spike</button>
          <button type="button" className="rounded-lg bg-sky-500/20 px-3 py-2 text-sm text-sky-100" onClick={()=>void runOp('Reply draft', () => draftOtareviewsReply({}))}>Reply draft</button>
          <button type="button" className="rounded-lg bg-emerald-500/20 px-3 py-2 text-sm text-emerald-100" onClick={()=>void runOp('Seed recovery', () => seedOtareviewsRecoveryOffer({}))}>Seed recovery offer</button>
          <button type="button" className="rounded-lg bg-obsidian-800 px-3 py-2 text-sm text-slate-200" onClick={()=>void runOp('Ack', () => ackOtareviewsFlag({ note: 'ui ack' }))}>Flag ack</button>
        </div>
      </PanelCard>
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchOtareviews(r.id,{status:'new'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>new</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchOtareviews(r.id,{status:'replied'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>replied</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchOtareviews(r.id,{status:'escalated'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>escalated</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
