...
import * as api from '../services/oracle'
          ...
          <PanelCard title={data.title}>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              <button
                ...
                onClick={() =>
                  void api.runOracleSweep({ force: true }).then((r: any) => {
              ...
              <button
                ...
                onClick={() =>
                  void api.resolveOracleAnomaly({}).then((r: any) => {
                    ping(`Anomaly resolve ${r.resolved?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.clearOracleScoreRed({}).then((r: any) => {
                    ping(`Score clear ${r.cleared?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.promoteOracleCanary({}).then((r: any) => {
                    ping(`Canary promote ${r.promoted?.length ?? 0}`)
                    return refresh()
                  })
                }
              >
              ...
              <button
                ...
                onClick={() =>
                  void api.ackOracleFlag({}).then((r: any) => {
            ...
            <p className="mt-2 text-xs text-slate-500">
              Flag {data.summary?.flags_open ?? 0} · anomaly {data.summary?.anom_open ?? 0} · red{' '}
              {data.summary?.score_red ?? 0} · canary {data.summary?.flag_canary ?? 0}
            </p>
          </PanelCard>
              ...
              {(data.flags || []).slice(0, 10).map((f: any) => (
                <li key={f.id} className="flex items-center justify-between rounded-lg border border-obsidian-700 px-3 py-2">
                  ...
                  <button
                    ...
                    onClick={() =>
                      void api.ackOracleFlag({ id: f.id }).then(() => {
                        ping('Ack')
                        return refresh()
                      })
                    }
