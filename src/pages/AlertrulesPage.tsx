...
import { createAlertrules, fetchAlertrules, patchAlertrules } from '../services/alertrules'
export default function AlertrulesPage() {
  const [rows, setRows] = useState<any[]>([])
  const [error, setError] = useState<string | null>(null)
  const [form, setForm] = useState<Record<string,string>>({"name":"Low stock","channel":"push"})
  async function refresh() {
    try { const data = await fetchAlertrules(); setRows(data.alertrules || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createAlertrules(payload); await refresh()
  ...
  return (
    <div className="space-y-6 p-6">
      <header>
        ...
        <p className="mt-1 text-sm text-slate-400">Bildirim kural tanımı.</p>
      </header>
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchAlertrules(r.id,{status:'enabled'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>enabled</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchAlertrules(r.id,{status:'disabled'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>disabled</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
