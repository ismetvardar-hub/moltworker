...
export default function RouteplanPage() {
  ...
  const [form, setForm] = useState<Record<string,string>>({"from":"Resort","to":"Antalya"})
  async function refresh() {
    try { const data = await fetchRouteplan(); setRows(data.routeplan || []); setError(null) }
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      await createRouteplan(payload); await refresh()
        ...
        <ul className="space-y-2 text-sm">
          {rows.map((r)=>(
            <li key={r.id} className="flex flex-wrap items-center justify-between gap-2 rounded-lg border border-obsidian-700 px-3 py-2">
              ...
              <div className="flex flex-wrap gap-1"><button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchRouteplan(r.id,{status:'draft'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>draft</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchRouteplan(r.id,{status:'live'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>live</button>
                <button type="button" className="rounded-md bg-obsidian-800 px-2 py-1 text-[10px] text-slate-300" onClick={()=>void patchRouteplan(r.id,{status:'done'}).then(()=>refresh()).catch(e=>setError(String(e.message||e)))}>done</button></div>
            </li>
          ))}
        </ul>
      </PanelCard>
